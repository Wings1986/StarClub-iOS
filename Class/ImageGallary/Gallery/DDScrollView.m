...
#ifdef SHOW_PAGE_CONTROL
      CGRect rect = CGRectMake(0, self.height - 40, self.width, 40);
#else
      CGRect rect = CGRectMake(0, self.height, self.width, 40);
#endif      
...
//	  pageControl.backgroundColor = [UIColor colorWithRed:193.0/255.0 green:157.0/255.0 blue:181.0/255.0 alpha:0.3];
...
- (void)dealloc {
 ...
 // [pageControl release];
  ...
  DD_RELEASE_SAFELY(_animationStartTime);
  DD_RELEASE_SAFELY(_pages);
  DD_RELEASE_SAFELY(_pageQueue);
...
#ifdef SHOW_PAGE_CONTROL
    return CGRectMake(-xd/2, -yd/2, width, height - 40);
#else
...
- (void)enqueueAllPages {
  for (NSInteger i = 0; i < (int)_pages.count; ++i) {
...
- (void)touchesMoved:(NSSet*)touches withEvent:(UIEvent *)event {
  ...
  DD_INVALIDATE_TIMER(_holdingTimer);
  ...
  if (_visiblePageIndex != _centerPageIndex && self.centerPage) {
    ...
    [_delegate scrollView:self didMoveToPageAtIndex:_centerPageIndex];
	  ...
	  if (pageControlUsed==NO) {
		  pageControl.currentPage = self.centerPageIndex;
	  }
  }
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setDataSource:(id<DDScrollViewDataSource>)dataSource {
  _dataSource = dataSource;
//	pageControl.numberOfPages = [_dataSource numberOfPagesInScrollView:self];
  [self reloadData];
}
...
- (NSInteger)numberOfPages {
    int pages = [_dataSource numberOfPagesInScrollView:self];

...
@interface AllAccessController ()<CommentAutoControllerDelegate, UIActionSheetDelegate,
                AddTextContollerDelegate, AddPhotoContollerDelegate, AddVideoContollerDelegate>
{
    int m_nIndexDel;
    
    BOOL m_bFirstStarName;
}
...
@implementation AllAccessController
...
- (void)viewDidLoad
{
    ...
    [ StarTracker StarSendView: @"All Access"];
 
    if ([Global getUserType] == ADMIN_CMS || [Global getUserType] == ADMIN_POST) {
        ...
        [btnEdit setImage:[UIImage imageNamed:@"btn_update"] forState:UIControlStateNormal];
        [btnEdit addTarget:self action:@selector(onUpdate) forControlEvents:UIControlEventTouchUpInside];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(startRefresh) name:NOTI_SHOW_MAINFEED object:nil];
...
{
        NSLog(@"2=======================");
    NSString * urlString = [MyUrl getMainFeedUrl:@"all" page:m_nPage];
//    NSString * urlString = [MyUrl getMainFeedUrl:@"all"];
    
//    NSData * pinResult = [NSData dataWithContentsOfURL:[NSURL URLWithString:urlString]];
...
//    NSDictionary * result = [NSJSONSerialization JSONObjectWithData:pinResult options:NSJSONReadingMutableContainers error:nil];
...
- (void) doneLoadingTableViewData {
    ...
    if (!m_bFirstStarName) {
        m_bFirstStarName = YES;
        
        NSString * adminName = [self.starInfo objectForKey:@"name"];
        
        GHAppDelegate * appDelegate = APPDELEGATE;
        [appDelegate changeHeaderName:adminName];
    }
...
-(void) onUpdate
{
    UpdateController * pController = [[UpdateController alloc] init];

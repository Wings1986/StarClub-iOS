...
- (void)renderMainMenu {
	...
	UIFont *buttonFont = [UIFont fontWithName:FONT_NAME size:15.0];
	...
	for (NSString *mainTitle in self.mainItemTitles) {
        CGSize titleTextSize = [mainTitle sizeWithAttributes:@{NSFontAttributeName:buttonFont}];
...
//    UIImageView *mainBackgroundImageView = [[UIImageView alloc] initWithImage:resizedBgImage];
    UIImageView *mainBackgroundImageView = [[UIImageView alloc] init];
    mainBackgroundImageView.backgroundColor = [UIColor clearColor];
...
- (UIView *)renderSubmenuAtIndex:(NSUInteger)index {
	...
	if ([self.submenus objectForKey:@(index)]) {
		...
		UIFont *buttonFont = [UIFont fontWithName:FONT_NAME size:13.0];
//		UIFont *mainItemFont = [UIFont fontWithName:FONT_NAME_BOLD size:13.0];
		...
		for (NSString *subTitle in subtitles) {
            CGSize titleTextSize = [subTitle sizeWithAttributes:@{NSFontAttributeName:buttonFont}];
...
//		f.size = CGSizeMake(itemWidth*columns + self.contentInsets.left + self.contentInsets.right, (mainItemPosition+1)*itemHeight + self.contentInsets.top + self.contentInsets.bottom);
        f.size = CGSizeMake(itemWidth*columns + self.contentInsets.left + self.contentInsets.right, (mainItemPosition)*itemHeight + self.contentInsets.top + self.contentInsets.bottom);
...
//    CGSize titleTextSize = [newTitle sizeWithAttributes:@{NSFontAttributeName:buttonFont}];
    ...
    CGRect rect = [attributedText boundingRectWithSize:(CGSize){1000, 1000}
...
//	menuFrame.origin.x -= diff;
	...
	for (NSInteger i = itemIndex+1; i < (int)framesCount; i++) {
	...
	[UIView animateWithDuration:self.animationDuration animations:^{
		...
		for (NSInteger i = itemIndex; i < (int)framesCount; i++) {
			CGRect fi = [[self.mainItemFrames objectAtIndex:i] CGRectValue];
...
- (void)reset {
	...
	for (NSInteger i=0; i < (int)[self.mainItemTitles count]; i++) {
